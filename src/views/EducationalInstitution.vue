<template>
  ...
  <div class="container d-lg-flex">
    <fade-transition>
      <div v-if="this.selectedRegion" class="me-4">
        <strong class="d-block">{{ selectedRegion?.name }}</strong>
  ...
  <p class="text-center" v-if="!educationalInstitutions.length">
    Damaged educational institution is not added yet!
  </p>
</template>
    ...
    async fetchEducationalInstitutions(perPage, pageNo) {
      try {
        ...
        const response = await apiClient.get(
          `/api/user_posts?type=education&&per_page=${perPage}&&page_no=${pageNo}&&region=${this.regionIdForFilter}&&zone=${this.zoneIdForFilter}`
        );
        if (response.status === 200) {
